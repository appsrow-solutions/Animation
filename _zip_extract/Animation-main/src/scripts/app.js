...
export default class Sketch {
  ...
  loadCardVideo(url, material) {
    ...
    // Full file into memory once — pause/resume never hits the network again
    fetch(url)
      .then((response) => {
  ...
  updateVideoPlayback(now = performance.now()) {
    ...
    for (let i = 0; i < this.videoElements.length; i++) {
      ...
      ranked.push({ entry, inBand, dist: Math.abs(worldY) });
    }
  ...
  updateUniforms(time) {
    ...
    this.scrolling = now - this.lastScrollAt < SCROLL_IDLE_MS;
    ...
    // Freeze ripple clock while scrolling so mesh can't shimmer mid-fold
    const shaderTime = this.scrolling
      ? (this._scrollFreezeTime ?? time)
      : time;
    if (!this.scrolling) this._scrollFreezeTime = time;
